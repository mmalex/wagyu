// bit banged i2c due to lack of strong pullups (doh)

// scl is PB8, sda is PB9
static inline void SCL1(void) 	 { GPIOB->ODR|=(1<<8); GPIOB->MODER|=(1<<16); }
static inline void SDA1(void) 	 { GPIOB->ODR|=(1<<9); GPIOB->MODER|=(1<<18); }
static inline void SDAIn(void) { GPIOB->ODR|=(1<<9); GPIOB->MODER&=~(1<<18); }
static inline void SCL0(void) 	 { GPIOB->ODR&=~(1<<8); GPIOB->MODER|=(1<<16);}
static inline void SDA0(void) 	 { GPIOB->ODR&=~(1<<9); GPIOB->MODER|=(1<<18);}
volatile int i2cd=0;
static inline void ShortDelay(void) { for (int j=0;j<10000;++j) i2cd++; }
static inline int ReadSDA(void) { return ((GPIOB->IDR>>9)&1); }
...
uint8_t wmcodec_write( uint8_t reg, uint16_t data )
{
	...
	SCL0();
	ShortDelay();
	uint32_t b=(0x34<<16)+(reg<<9)+(data & 0x1ff);
    uint8_t ack=0;
	...
	ShortDelay();
	if (ack) {
		DebugLog("oh no we got a nack reg=%02x data=%03x\r\n", reg, data);
	}
	return ack;
}


// constants transcribed from datasheet from
//	https://github.com/mguentner/rockbox/blob/master/firmware/drivers/audio/wm8758.c
...
void codec_init(void) {
    wmcodec_write(0,0);
	    ...
	    wmcodec_write(PWRMGMT2, PWRMGMT2_ROUT1EN | PWRMGMT2_LOUT1EN | PWRMGMT2_BOOSTENL | PWRMGMT2_BOOSTENR | PWRMGMT2_ADCENL | PWRMGMT2_ADCENR);
	    ...
	    wmcodec_write(LOUT1VOL, 0x100+voldb+57);
	    wmcodec_write(ROUT1VOL, 0x100+voldb+57);
